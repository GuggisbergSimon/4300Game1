...
public class Projectile : MonoBehaviour {
    ...
    private void OnTriggerEnter2D(Collider2D collision) // Allows the script to know what object the projectile has collided with.
    {
    ...
    private void Start()
    {
        ...
        projectileDirection = Vector3.Normalize(GameManager.Instance.projectileSpawner.transform.position - GameManager.Instance.player.transform.position); // Set up once, gives the projectile the direction in which to fly: (1,0,0) when facing right, (-1,0,0) when facing left.
    }
